...
<label for="payid">Payroll Id:</label>
...
<input type="time" id="overt" name="Overt" required><br><br>

<label for="ded">Deductions:</label>
<input type="number" id="ded" name="ded" required><br><br>

<label for="netp">Net_Pay:</label>
<input type="number" id="netp" name="netp" required><br><br>
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $stmt = $connection->prepare("INSERT INTO Payroll(Payroll_Id, Employee_Id , Salary , Overtime,Deductions,Net_Pay) VALUES (?,?,?,?,?,?)");
    $stmt->bind_param("ssssss", $payid, $empid,$salary,$Overt,$ded, $netp);
...
<head>
    <meta charset="utf-8">
    <meta name="viewport" content="width=device-width, initial-scale=1">
    <title>Detail informations Of Payroll</title>
    <style>
        table {
            width: 100%;
...
<body>
    ...
    <table border="5">
        ...
        // Prepare SQL query to retrieve all Payroll
        $sql = "SELECT * FROM Payroll";
        $result = $connection->query($sql);

        // Check if there are any Payroll
        if ($result->num_rows > 0) {
            // Output data for each row
            while ($row = $result->fetch_assoc()) {
                $pid = $row['Payroll_Id']; // Fetch the Payroll_Id
                echo "<tr>
                    ...
                    <td>" . $row['Net_Pay'] . "</td>
                    <td><a style='padding:4px' href='delete_payroll.php?Payroll_Id=$pid'>Delete</a></td> 
                    <td><a style='padding:4px' href='update_payroll.php?Payroll_Id=$pid'>Update</a></td> 
                </tr>";
            }
        } else {
